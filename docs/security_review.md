...
**Goal:** Fail closed in non-test environments unless a sufficiently long, non-public `JWT_SECRET_KEY` is configured, so the application never runs in a dangerously ambiguous signing state.
...
3. Reject known committed example or placeholder values even when they are long enough.
